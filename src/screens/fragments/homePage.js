...
import { getAlarm, getInfo } from "../../data/api";
...
const HomePage = () => {
  ...
  const [alarms, setAlarms] = React.useState([
    {
      // alarm_message: "lsakdlas;kd;laskdl;kdl;askdl;akdsal;kda;lkdal;",
      // has_alarm: true,
    },
  ]);
  ...
  const myFunction = React.useCallback(() => {
    ...
    getAlarm().then((res) => {
      setAlarms([]);
      console.log(res.has_alarm);
      setAlarms([
        { alarm_message: res.alarm_message, has_alarm: res.has_alarm },
      ]);
    });
  }, [alarms]);
  React.useEffect(() => {
  ...
  return (
    <Container>
      ...
      >
        ...
        >
          <MyTitle
            style={{ marginLeft: "20px", marginTop: "30px", fontSize: "32px" }}
          >
            Alarms
          </MyTitle>
          <div style={{ width: "100%", marginTop: "20px", overflow: "auto" }}>
            {alarms.length !== 0 ? (
              ...
              <AlarmTextContainer>
                <AlarmState color={"rgba(32, 140, 1, 1)"} />
                <AlarmText>No Alarm</AlarmText>
              </AlarmTextContainer>
            )}
          </div>
        </Card>
